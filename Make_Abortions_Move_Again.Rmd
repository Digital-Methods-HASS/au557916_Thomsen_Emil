...
ggplot(subset(final_df2, Year == 2018), aes(Abor_pr_capita20_24, Municipalities, size = Income_pr_capita)) + 
  geom_point()
...
server <- function(input, output) {
   output$distPlot <- renderPlotly({
      qplot(Abor_pr_capita, Year, size = Income_pr_capita, color = Municipalities, data = final_df2) + 
      geom_point()
   })
}
...
#Cool visualisation that shows all the municipalities. and all years.
...
server <- function(input, output) {
   output$distPlot <- renderPlotly({
      ggplot(subset(final_df, Year == 2018), aes(x=Municipalities, y=Abor_pr_capita, color =  Income_pr_capita)) +
...
```{r, Q2}
final_df2 %>%  #We choose to look at the gapminder dataset and add pipes to it, so we can send an output of one fucntion directly to another.
  filter(Year == 2018) %>%  #First we filter the years in tha dataset so vi are only looking at data from 1952
  select(Municipalities , Abor_pr_capita) %>% #then we select or choose to look at the countries and their gdpPercap, as everything else is irelevant right now
  arrange(desc(Abor_pr_capita)) #lastly, we arange our data in a decending order so we have the richest nation first.
...
final_df2 %>%  #We choose to look at the gapminder dataset and add pipes to it, so we can send an output of one fucntion directly to another.
  ...
  arrange(desc(Income_pr_capita))

```

```{r}

søjle <- ggplot(subset(final_df, Year == 2018), aes(x=Municipalities, y=Abor_pr_capita, color =  Income_pr_capita)) +
  geom_bar(stat="identity")
...
```{r}
options(scipen=999) #By adding this line of code we disable the scientific way of writing lables.
ggplot(final_df2, aes(x = Income_pr_capita, y = Abor_pr_capita, size = Income_pr_capita))+
  geom_point() + 
 # scale_x_log10() + 
  transition_time(Year) + 
  labs(title = "{closest_state}", x = "Income per capita", y = "Abortions per capita") + 
