...
```{r}
unique(gapminder$year) #Vi kigger på de forkomster af af unikke år der er i datasettet gapminder
head(gapminder) #Her kigges på starten af Gapminder 
tail(gapminder)
...
ggplot(subset(gapminder, year == 1952), aes(gdpPercap, lifeExp, size = pop)) + #Vi bruger ggplot til at lave en visualisering, så bruger vi subset til at sige at vi vil kigge på året 1952 i gapminder datasettet.
  # aes bruges til at sætte æstetikken, hvor vi siger at gdpPercap skal være x-aksen og lifeExp skal være y. Størelsen på plotsne skal bestemmes af pop (population) dvs. des større pop des større prik i visualiseringen.
...
```{r Q7}
# Suedo kode: 
# Life expectancy ud af x-aksen
# år op af y-aksen
#lav det i et søjlediagram
